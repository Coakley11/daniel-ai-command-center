...
def _migrate_account_columns(conn: sqlite3.Connection) -> None:
    ...
    if _has_column("app_current_state", "app") and (
        not _has_column("app_current_state", "user_id") or _pk_columns("app_current_state") == ["app"]
    ):
        conn.execute(
            """
        ...
        if _has_column("app_current_state", "user_id"):
            conn.execute(
                """
                INSERT OR REPLACE INTO app_current_state_v2
                    (user_id, app, page, summary, metrics_json, updated_at)
                SELECT user_id, app, page, summary, metrics_json, updated_at FROM app_current_state
                """
            )
        else:
            conn.execute(
    ...
    if _has_column("resume_items", "user_id") and _resume_items_needs_user_unique(conn):
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS resume_items_v2 (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id TEXT NOT NULL,
                app TEXT NOT NULL,
                item_key TEXT NOT NULL,
        ...
        conn.execute(
            """
            INSERT OR REPLACE INTO resume_items_v2
                (user_id, app, item_key, title, subtitle, action_url, valid, updated_at)
            SELECT user_id, app, item_key, title, subtitle, action_url, valid, updated_at
            FROM resume_items
            """
        )
        conn.execute("DROP TABLE resume_items")
        conn.execute("ALTER TABLE resume_items_v2 RENAME TO resume_items")


def _resume_items_needs_user_unique(conn: sqlite3.Connection) -> bool:
    """True when resume_items UNIQUE is still (app, item_key) from pre-account schema."""
    col_rows = conn.execute("PRAGMA table_info(resume_items)").fetchall()
    if not any(str(r[1]) == "user_id" for r in col_rows):
        return False
    col_names = [str(r[1]) for r in col_rows]
    for idx in conn.execute("PRAGMA index_list(resume_items)").fetchall():
        if not idx[2]:
            continue
        idx_cols = [
            col_names[int(info[1])]
            for info in conn.execute(f"PRAGMA index_info({idx[1]})").fetchall()
        ]
        if idx_cols == ["app", "item_key"]:
            return True
    return False
...
def _sqlite_save_current_state(
    ...
    *,
    ...
    with _connect() as conn:
        conn.execute(
            """
            INSERT INTO app_current_state (user_id, app, page, summary, metrics_json, updated_at)
            VALUES (?, ?, ?, ?, ?, ?)
